...
export const ProjectCard = ({ project }: { project: ProjectType }) => {
    const { title, description, thumbnail, status, technologies, links, ytVideoLink } = project;
    ...
    return (
        <>
            <div className="border border-zinc-900 hover:border-zinc-800 rounded-xl overflow-hidden transition-colors group">
                {/* Thumbnail */}
                    ...
                    {videoId && (
                        ...
                        >
                            <span className="flex items-center justify-center w-14 h-14 rounded-full bg-white/90 group-hover:scale-110 transition-transform shadow-lg">
                                <Play className="w-6 h-6 text-black fill-black ml-0.5 cursor-pointer" />
                            </span>
                        </button>
                    )}
                </div>
